...
import { openAction } from "../util/appMenuSlice";
import { cacheData } from "../util/searchSlice";

const Header = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [searchSuggestions, setSearchSuggestions] = useState([]);
  const [isShowSuggestion, setIsShowSuggestion] = useState(false);
  const cacheSearchData = useSelector((store) => store.search);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (cacheSearchData[searchQuery]) {
        setSearchSuggestions(cacheSearchData[searchQuery]);
      } else {
        getSearchData();
      }
    }, 200);
  ...
  return (
    <div className="grid grid-flow-col shadow-lg">
      <div className="flex col-span-1">
        <img
          className="w-22 h-12 p-2 m-2 cursor-pointer"
          alt="menu"
          onClick={() => handleState()}
      ...
      <div className="col-span-10 my-4 ml-24">
        <input
          className="w-96 px-3 h-10 border border-gray-400"
          type="search"
          value={searchQuery}
        ...
        />
        <span className="bg bg-gray-300 h-24 w-12 p-2">🔍</span>
        <div className="absolute bg bg-white w-96 shadow-2xl rounded-sm p-2 ">
          <ul>
            {isShowSuggestion &&
              searchSuggestions.map((suggestion, index) => (
                <li className="hover:bg-gray-200 p-1" key={index}>
                  {suggestion}
                </li>
              ))}
          </ul>
      ...
      <div className="col-span-1">
        <img
          className="w-26 h-12"
          alt="signin"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRHAgKES15I0EMr88Ac7DBjfmvJvvcMuTSYgw&usqp=CAU"
        />
      </div>
    </div>
  );
